...
* 4. 파이썬 logging 모듈 (module) 사용하여 로그 기록하기
...
# 1 단계 - 전역 로그 객체(logger) 설정 - logger의 이름(__name__)을 명시해서 전역 로그 객체(logger) 설정하기
...
# 4 단계 - 아마존 웹서비스 람다 함수(AWS Lambda Function)의 최상위 로그 객체(root)와 분리하기 위해 propagate 속성(property) 값을 False로 설정 (False일 경우, chatbot_logger의 로깅 메시지가 최상위 로그 객체(root)의 처리기로 전달되지 않는다.)
logger.propagate = False

# 5 단계 - formatter 생성 (로그 기록/출력/저장에 사용할 날짜 + 로그 메시지)
...
# 6 단계 - handler 생성 (streamHandler: 콘솔 출력용 // fileHandler: 파일 기록용)
# streamHandler = logging.StreamHandler()
streamHandler = logging.StreamHandler(sys.stdout)

# fileHandler 생성 및 전역 로그 객체(logger)에 handler 추가 후 OpenAI 기능 함수 openAI.getMessageFromGPT(prompt) 실행시
# 카카오톡 채팅방에 아래와 같은 메시지가 추가로 출력되어 나오므로 fileHandler 생성 및 전역 로그 객체(logger)에 handler 추가하는 로직 주석 처리 + OpenAI 기능 모듈 (module)(openAI.py) 삭제 처리 진행 (2025.09.19 minjae)
...
# fileHandler = logging.FileHandler(chatbot_helper._tmp + chatbot_helper._chatbot_file_name)    # 아마존 웹서비스 람다 함수(AWS Lambda Function)에 로그 기록할 파일 이름(경로) "/tmp/botlog.txt" 지정 (파일 이름은 다른 것으로 변경해도 된다.)

# 7 단계 - 각각의 Handler에 formatter 설정(할당) 적용
streamHandler.setFormatter(formatter)
# fileHandler.setFormatter(formatter)

# 8 단계 - 전역 로그 객체(logger)에 handler 추가 (로그 기록시 해당 handler 적용)
logger.addHandler(streamHandler)
# logger.addHandler(fileHandler)

"""
*** 참고 ***
파이썬 logging 모듈 (module) 사용하여 전역 로그 기능 구현
...
챗봇 전역 로그 모듈 (module) 구현 순서 및 로그 기록 방법 
...
7. 다른 파이썬 스크립트 파일에서 챗봇 전역 로그 모듈 (module) import 처리 - from modules.log import logger   # 챗봇 전역 로그 객체(logger)  
