...
async def get_masterDownLoadAsync(masterEntity_json_file_path: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        with open(masterEntity_json_file_path, 'r', encoding='utf-8') as json_file:
            ...
            await asyncio.sleep(0.1)    # 비동기 함수 (async/await) 컨텍스트 유지 (필요 시 삭제 가능) -> 0.1초 대기. asyncio.sleep 비동기 함수도 네이티브 코루틴이다. (async def로 만든 코루틴은 네이티브 코루틴이라고 한다.)
...
* finally문에서 return문 사용 금지 (사유: try 및 except에서 사용되는 return문 무시되고 없던 일이 되어버리는 치명적 버그 발생하기 때문.)
...
3. 2번에서 리턴 받은 master_datas 가지고 싱글톤 (singleton) 패턴 전용 모듈 (singleton.py) -> 데이터 유효성 검사 (self.__isValidator()) 진행
