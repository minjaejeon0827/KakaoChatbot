...
# 2026-01-21 Ubuntu linux 4차 테스트 커밋

# 1. 공통 모듈 (module) 먼저 import
from commons import chatbot_helper   # 챗봇 전용 도움말 텍스트

# 2. singleton 모듈 (module) log 모듈 (module) 보다 먼저 import
from modules.singleton import MasterEntity   # 마스터 데이터 싱글톤 클래스
from modules.chatbot_enum import EnumValidator   # 데이터 유효성 검사

# 3. singleton 모듈 (module) 먼저 초기화 된 후 log 모듈 (module) import
...
messageText_mappings = {   # 챗봇 버튼 메시지 텍스트 매핑 딕셔너리
    ...
    chatbot_helper._ai_assistant: chatbot_helper._ai_assistant,   # level1 - AI Assistant
...
masterEntity = MasterEntity(messageText_mappings, valid_targets)   # 마스터 데이터 싱글톤 클래스 인스턴스
kakaoResponseFormatter = KakaoResponseFormatter(masterEntity.get_master_datas, masterEntity.get_messageText_mappings)   # 스킬 응답 템플릿 json 포맷 클래스 인스턴스
