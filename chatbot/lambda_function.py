    ...
    try:
        ...
        if None is result:
            ...
            return lambda_response_format(
                ...
                status_code=chatbot_helper._statusCode_success,
            )
        ...
        valid_error_payload = error_payload_format(chatbot_helper._error_title, str(e))
        return lambda_response_format(valid_error_payload)
        ...
        sys_error_payload = error_payload_format(chatbot_helper._error_title, str(e))
        return lambda_response_format(sys_error_payload)
...
def parse_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"[테스트] event['body'] json 파싱 오류 - {parsing_error_msg}", exc_info=True)
        raise
    except (KeyError, ValueError, TypeError) as e:
        valid_error_msg = str(e)
        logger.error(f"[테스트] 데이터 유효성 오류 - {valid_error_msg}", exc_info=True)
...
def thread_wrapper(target, args: tuple[Any, ...], res_queue: Queue, err_queue: Queue) -> None:
    ...
    try:
        ...
        logger.critical(f"[테스트] 작업 쓰레드 오류 - {thread_error_msg}", exc_info=True)
        err_queue.put(thread_error_msg)
