# Dockerfile 명령어
# 코드 리뷰 참고 URL - https://chatgpt.com/c/691d6f71-6a90-8321-951f-e270ab4aa753
...
# requirements.txt만 우선 복사 (캐싱 최적화)
COPY requirements.txt .

# python 패키지를 /tmp/python 에 설치 (AWS Lambda 표준 구조)
...
# Stage 1에서 빌드한 패키지들만 복사 (가볍고 빠른 런타임)
COPY --from=build /tmp/python /var/task/

# 애플리케이션 코드만 복사 (.dockerignore 파일에 작성된 폴더, 파일은 복사 대상 제외!)
COPY . .
...
# *** 참고 ***
# *** 카카오 챗봇 배포 순서 ***
# 1 단계: Docker 카카오 챗봇 이미지 배포 - 아마존 웹서비스 ECR (AWS Elastic Container Registry)
...
# 1. 파일 탐색기 실행 -> 폴더 경로 (예) "D:\minjae" 이동 -> 키보드 "Shift" 키 누르고 -> 폴더 (예) "KakaoChatbot" 마우스 우클릭 -> "여기에 PowerShell 창 열기(S)" 클릭
...
#    -> 명령어 type $env:USERPROFILE\.aws\credentials 입력 및 엔터 -> 아래처럼 액세스키, 비밀 (Secret) 액세스 키 출력
#       [default]
