...
def basicCard_json(master_data: dict, buttons: list[dict]) -> dict:
    ...
    # master_data[chatbot_helper._botRes]에 할당된 값이 null 또는 공백("")일 경우 basicCard 가 카카오톡 채팅방에 출력 안되는 오류 발생함. 
    ...
    if master_data[chatbot_helper._botRes]:   # master_data[chatbot_helper._botRes]에 할당된 값이 null 또는 공백("")이 아닌 경우 (None or Empty String Check)
        outputs.append({
            "simpleText": {
                "text": master_data[chatbot_helper._botRes]
            }
        })
...
def get_response(userRequest_msg: str, masterEntity: dict) -> tuple[dict, dict]:
    ...
    try:
        ...
        if (chatbot_helper._start in userRequest_msg or chatbot_helper._beginning in userRequest_msg):   # start - 시작 화면 or 처음으로
            ...
            (response, master_data) = base_response(master_datas[chatbot_helper._startCard])
        ...
        # 오류 메시지 - "TypeError: 'NoneType' object is not subscriptable"
            ...
            (response, master_data) = base_response()
