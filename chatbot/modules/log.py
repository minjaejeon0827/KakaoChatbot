...
logger = logging.getLogger(chatbot_helper._chatbot_logger)   # 최상위 로그 객체(root) 대신 특별한 이름('chatbot_logger')의 전역 로그 객체(logger) 생성
...
# fileHandler = logging.FileHandler(chatbot_helper._tmp + chatbot_helper._chatbot_file_name)    # 아마존 웹서비스(AWS) 람다 함수(Lambda Function)에 로그 기록할 파일 이름(경로) "/tmp/botlog.txt" 지정 (파일 이름은 다른 것으로 변경해도 된다.)
