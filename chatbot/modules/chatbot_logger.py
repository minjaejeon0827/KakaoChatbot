...
def _callerInfo() -> tuple[str, str, int]:
    ...
    for frame_info in stack:   # 호출 stack 역순으로 순회 및 chatbot_logger.py 모듈 밖 상위 호출자 자동 탐색.
        module = inspect.getmodule(frame_info.frame)  # 해당 프레임 객체(frame_info.frame) 어느 모듈(module)에서 생성되었는지 추적
        file_name = os.path.basename(frame_info.filename)
        cur_file_name = os.path.basename(__file__)   # chatbot_logger.py 파일 이름
    ...
    return ("unknown_file", "unknown_function", -1)  # chatbot_logger.py 모듈 밖 상위 호출자 찾기 불가
...
def _logWrite(level: str, msg: str) -> None:
    ...
    if "unknown" is file_name or "unknown" is function_name or -1 is lineno:   # 상위 호출자 존재 안 하는 경우
        print("[%s] [%s] [%s | %s - L%s]: %s" %(level, time_stamp, file_name, function_name, lineno, 'chatbot_logger.py 모듈 밖 상위 호출자 찾기 불가!'))
