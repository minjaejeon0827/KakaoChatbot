...
# class KakaoResponseFormat(object):  # 명시적으로 object 클래스 상속
class KakaoResponseFormat:   # 암시적으로 object 클래스 상속
    """
    Description: 스킬 응답 템플릿 json 포맷 클래스
    ...
    def __init__(self, master_datas):
        """
        Description: 생성된 객체 초기화

                     *** 주요 특징 ***
                     1. 객체 생성 시 전달된 모든 인자(master_datas 제외)를 __new__ 메서드가 먼저 받고, 그 다음 __init__ 메서드로 전달
                     2. 생성된 객체에 속성(property) 추가 및 값 할당 

                     참고 URL - https://docs.python.org/ko/3.6/reference/datamodel.html#object.__init__

        Parameters: self (object) - 스킬 응답 템플릿 json 포맷 클래스 (KakaoResponseFormat) 인스턴스 (Instance)
...
def __skillTemplate_format(outputs: list[dict] = [], quickReplies: list[dict] = []) -> dict[str, Any]:
    """
...
def __quickReplies_format(master_data: dict[str, Any], quickReplies: list[dict]) -> dict[str, Any]:
    """
    ...
    return __skillTemplate_format(outputs, quickReplies)

def __textCard_format(master_data: dict[str, Any], buttons: list[dict]) -> dict[str, Any]:
    """
    ...
    return __skillTemplate_format(outputs)

def __basicCard_format(master_data: dict[str, Any], buttons: list[dict]) -> dict[str, Any]:
    """
...
def __carousel_format(master_data: dict[str, Any], items: list[dict]) -> dict[str, Any]:
    """
...
def __empty_response(master_data: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    empty_format = __skillTemplate_format()

    return { "format": empty_format, "meta_data": master_data }

def __create_buttons(master_data: dict[str, Any], message_prefix: str = None) -> list[dict]:
    """
...
def __create_quickReplies(master_data: dict[str, Any], message_prefix: str = None) -> list[dict]:
    """
...
def __common_basicCard(master_data: dict[str, Any]) -> dict[str, Any]:
    """
    Description: [공통] 기본형 카드 json 포맷 가져오기 

    Parameters: master_data - 특정 마스터 데이터

    Returns: __basicCard_format(master_data, buttons) - [공통] 기본형 카드 json 포맷
             master_data - 특정 마스터 데이터
    """

    buttons = __create_buttons(master_data)   # [공통] 버튼 리스트 생성

    return { "format": __basicCard_format(master_data, buttons), "meta_data": master_data }

def __common_quickReplies(master_data: dict[str, Any]) -> dict[str, Any]:
    """
    Description: [공통] 바로가기 그룹 json 포맷 가져오기

    Parameters: master_data - 특정 마스터 데이터

    Returns: __quickReplies_format(master_data, quickReplies) - [공통] 바로가기 그룹 json 포맷
             master_data - 특정 마스터 데이터
    """

    quickReplies = __create_quickReplies(master_data)

    return { "format": __quickReplies_format(master_data, quickReplies), "meta_data": master_data }

def __common_ver_quickReplies(userRequest_msg: str, master_data: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    Returns: __quickReplies_format(master_data, quickReplies) - [공통] Autodesk or 상상진화 BOX 제품 버전 바로가기 그룹 json 포맷
             master_data - 특정 마스터 데이터
    """

    message_prefix = f"{chatbot_helper._instType} {userRequest_msg}"
    quickReplies = __create_quickReplies(master_data, message_prefix)

    return { "format": __quickReplies_format(master_data, quickReplies), "meta_data": master_data }

def __subCat_basicCard(userRequest_msg: str, master_data: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    Returns: __basicCard_format(master_data, buttons) - 문의 유형 기본형 카드 json 포맷
             master_data - 특정 마스터 데이터
    """

    message_prefix = userRequest_msg
    buttons = __create_buttons(master_data, message_prefix)
...
def get_response(userRequest_msg: str, masterEntity: dict[str, Any]) -> dict[str, Any]:
    ...
    eq_operator_mappings = {   # if 조건절 eq 연산자(==) 매핑 Dictionary 객체
        ...
        chatbot_helper._revitBox: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._boxVerReplies]),
        chatbot_helper._cadBox: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._boxVerReplies]),
        chatbot_helper._energyBox: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._boxVerReplies]),

        # level4 - Autodesk 제품 버전
        chatbot_helper._autoCAD: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._adskVerReplies]),
        chatbot_helper._revit: lambda:  __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._adskVerReplies]),
        chatbot_helper._navisworksManage: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._adskVerReplies]),
        chatbot_helper._infraWorks: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._adskVerReplies]),
        chatbot_helper._civil3D: lambda: __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._adskVerReplies]) 
    }

    in_operator_mappings = {   # if 조건절 in 연산자 매핑 Dictionary 객체
        chatbot_helper._start: lambda: __common_basicCard(master_datas[chatbot_helper._startCard]),   # start - 시작 화면
        chatbot_helper._beginning: lambda: __common_basicCard(master_datas[chatbot_helper._startCard]),   # start - 처음으로
        chatbot_helper._remote_text: lambda: __empty_response(master_datas[chatbot_helper._startCard]),   # level1 - 원격 지원
        chatbot_helper._ask_chatbot: lambda: __chatbot_carousel(master_datas[chatbot_helper._chatbotCard]),   # level1 - 챗봇 문의
        chatbot_helper._adskProduct: lambda: __subCat_basicCard(userRequest_msg, master_datas[chatbot_helper._subCatCard]),   # level2 - 문의 유형 (Autodesk 제품)
        chatbot_helper._boxProduct: lambda: __subCat_basicCard(userRequest_msg, master_datas[chatbot_helper._subCatCard]),   # level2 - 문의 유형 (상상진화 BOX 제품)
        # end - 텍스트 + basicCard Autodesk or 상상진화 BOX 제품 설치 방법 매핑 Dictionary 객체
        f"{chatbot_helper._instType} {chatbot_helper._revitBox}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._revitBoxInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._cadBox}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._cadBoxInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._energyBox}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._energyBoxInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._autoCAD}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._autoCADInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._revit}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._revitInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._navisworksManage}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._navisworksManageInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._infraWorks}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._infraWorksInfos]),
        f"{chatbot_helper._instType} {chatbot_helper._civil3D}": lambda: __end_basicCard(master_datas[chatbot_helper._endCard], master_datas[chatbot_helper._endCard][chatbot_helper._civil3DInfos])
        ...
        # chatbot_messageTexts: lambda: conditional_action(userRequest_msg, chatbot_messageTexts, __subCat_basicCard(userRequest_msg, master_datas[chatbot_helper._subCatCard])),   # level2 - 문의 유형
        # adsk_messageTexts: lambda: conditional_action(userRequest_msg, adsk_messageTexts, __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._adskVerReplies])),   # level4 - Autodesk 제품 버전
        # box_messageTexts: lambda: conditional_action(userRequest_msg, box_messageTexts, __common_ver_quickReplies(userRequest_msg, master_datas[chatbot_helper._boxVerReplies])),   # level4 - 상상진화 BOX 제품 버전
    }
    ...
    try:
        ...
        return __empty_response(master_datas[chatbot_helper._emptyResponse])   # 기술지원 문의 제외 일반 문의
...
def __chatbot_carousel(master_data: dict[str, Any]) -> dict[str, Any]:
    """  
    Description: 챗봇 문의 아이템형 케로셀 json 포맷 가져오기

    Parameters: master_data - 특정 마스터 데이터

    Returns: __carousel_format(master_data, chatbot_items) - 챗봇 문의 아이템형 케로셀 json 포맷
             master_data - 특정 마스터 데이터
    """
    ...
    return { "format": __carousel_format(master_data, chatbot_items), "meta_data": master_data }

# TODO: 아래 함수 __end_basicCard 필요시 로직 수정 예정 (2025.09.05 minjae)
def __end_basicCard(master_data: dict[str, Any], endInfos: list[dict]) -> dict[str, Any]:
    """
    ...
    Returns: __skillTemplate_format(outputs) - 스킬 응답 템플릿 json 포맷
             master_data - 특정 마스터 데이터
    """
    ...
    return { "format": __skillTemplate_format(outputs), "meta_data": master_data }