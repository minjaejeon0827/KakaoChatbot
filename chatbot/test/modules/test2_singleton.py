...
class KSTFormatter(logging.Formatter):   # 명시적으로 logging.Formatter 클래스 상속
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(_class, "_init"):   # 해당 클래스에 _init 속성(property)이 없다면
            ...
            ----- super().__init__(*args, **kwargs)   # KSTFormatter 싱글톤 클래스의 부모 클래스 초기화자 super().__init__(*args, **kwargs) 실행 시 파라미터 datefmt 전달 인자 값이 None일 경우 로그 기록 형식 기본 값 할당 (default_time_format = '%Y-%m-%d %H:%M:%S')
