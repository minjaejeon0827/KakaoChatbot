...
# def conditional_action(userRequest_msg: str, masterEntity: dict) -> tuple[dict, dict]:
#     """
...
#     if userRequest_msg in chatbot_messageTexts:   # level2 - 문의 유형
#         logger.info(f"[테스트] [in_operator_mappings] chatbot_messageTexts - {chatbot_messageTexts}, userRequest_msg - {userRequest_msg}")
#         (response, master_data) = subCat_basicCard(userRequest_msg, master_datas[chatbot_helper._subCatCard])
#     else:   # 기술지원 문의 제외 일반 문의
#         logger.info(f"[테스트] [in_operator_mappings] 기술지원 문의 제외 일반 문의")
...
def get_response(userRequest_msg: str, masterEntity: dict) -> tuple[dict, dict]:
    ...
    try:
        ...
        elif chatbot_helper._remote_text in userRequest_msg:   # level1 - 원격 지원
            (response, master_data) = empty_response(master_datas[chatbot_helper._startCard])

        elif chatbot_helper._ask_chatbot in userRequest_msg:   # level1 - 챗봇 문의
        ...
        # elif chatbot_helper._ask_accountProduct in userRequest_msg:   # level3 - 계정 & 제품배정 문의
