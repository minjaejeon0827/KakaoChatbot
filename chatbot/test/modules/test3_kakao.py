...
def quickReplies_json(replies: dict, buttons: list[dict]) -> dict:
    """
...
def _create_buttons(master_data: dict, message_prefix: str = None) -> list[dict]:
    ...
    logger.info(f"[테스트] message_prefix - {message_prefix}")
    logger.info(f"[테스트] buttons - {buttons}")
...
def common_basicCard(card: dict) -> tuple[dict, dict]:
    ...
    buttons = _create_buttons(card)   # [공통] 버튼 리스트 생성
...
def common_quickReplies(replies: dict) -> tuple[dict, dict]:
    ...
    Returns: quickReplies_json(replies, buttons) - [공통] 바로가기 그룹 json 포맷
             replies - 특정 마스터 데이터
    """

    buttons = _create_buttons(replies)

    return (quickReplies_json(replies, buttons), replies)
...
def common_ver_quickReplies(userRequest_msg: str, verReplies: dict) -> tuple[dict, dict]:
    ...
    Returns: quickReplies_json(verReplies, buttons) - [공통] Autodesk or 상상진화 BOX 제품 버전 바로가기 그룹 json 포맷
             verReplies - 특정 마스터 데이터
    """

    message_prefix = f"{chatbot_helper._instType} {userRequest_msg}"
    buttons = _create_buttons(verReplies, message_prefix)

    return (quickReplies_json(verReplies, buttons), verReplies)
...
def get_response(userRequest_msg: str, masterEntity: dict) -> tuple[dict, dict]:
    ...
    try:
        ...
        # lambda_function.py 소스파일 -> chatbot_response 함수에서 res_queue.put(kakao_response[chatbot_helper._payload]) 실행할 때 발생하는 오류로 확인 되어 아래처럼 else 절 코드 추가 (2025.09.12 minjae)
