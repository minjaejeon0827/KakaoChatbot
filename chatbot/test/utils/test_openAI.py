...
def getMessageFromGPT(prompt): 
    ...
    try:
        ...
        error_msg = str(e)     
        chatbot_logger.openAI_log_write(chatbot_logger._error, "[테스트] 오류", error_msg) 
        raise
...
# def getChunksFromText(file_path):
...
#         # CharacterTextSplitter 클래스 객체 text_splitter 생성 
...
#         #     message = chatbot_helper._warningSSflex
         
#         # else:   # 관련 문서 리스트 존재하는 경우
#         #     response = retrieval_chain.invoke({"input": prompt})   # 질문하기(검색 수행)
#         #     message = response["answer"]   # 답변얻기
        
#         # logger.log_write(logger._info, '[테스트] 질문과 유사한 응답 메시지: ', message) 
...
#         raise    # raise로 함수 getMessageFromChunks의 현재 예외를 다시 발생시켜서 함수 getMessageFromChunks 호출한 상위 코드 블록으로 넘김    