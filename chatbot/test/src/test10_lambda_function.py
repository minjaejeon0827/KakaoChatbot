...
def handler(event: dict[str, Any], context: LambdaContext) -> dict[str, Any]:
    ...
    try:
        ...
        if None is res_queue:   # 챗봇 답변 내용 담을 큐 객체 res_queue 값이 None인 경우
            logger.info("[테스트] handler - finally 강제 종료 - [사유] 큐 객체 res_queue 생성 안 함.")
            return
             
        while(time.time() - start_time < chatbot_helper._time_limit):   # 챗봇 응답 시간 3.5초 이내인 경우
            if False == res_queue.empty():
                response = res_queue.get()  # 큐 객체 res_queue 에서 데이터 가져오기
                res_queue.task_done()   # 큐 객체 res_queue 에서 가져온 데이터에 대한 작업 완료
...
def chatbot_response(kakao_request: dict[str, Any], res_queue: q.Queue, file_path: str) -> None:
    """
    Description: 챗봇 답변 요청 및 큐 객체 res_queue 답변 내용 추가

    Parameters: kakao_request - 카카오톡 채팅방 실제 채팅 정보
                res_queue - 챗봇 답변 내용 담을 큐 객체
                file_path - 아마존 웹서비스 람다 함수(AWS Lambda Function) -> 임시 로그 텍스트 파일 상대 경로 ('/tmp/botlog.txt')
    ...
    try:
        ...
        # if '/error'== userRequest_msg:   # 오류 테스트
        #     raise Exception("사유: 오류 테스트!")
        
        # *** 참고 - masterEntity.get_isValid.name - Enum 열거형 구조체 EnumValidator 멤버변수 이름
        if EnumValidator.VALIDATION_ERROR == masterEntity.get_isValid:   # 데이터 유효성 검사 결과 오류인 경우
            raise ValueError("데이터 유효성 검사 오류.")
        
        if EnumValidator.NOT_EXISTENCE == masterEntity.get_isValid:   # 마스터 데이터 유효성 검사 결과 실패한 경우
            raise ValueError("마스터 데이터 존재 안 함.")

        # if True == masterEntity.get_isValid:   # 마스터 데이터 유효성 검사 결과 성공한 경우
        if EnumValidator.EXISTENCE == masterEntity.get_isValid:   # 마스터 데이터 유효성 검사 결과 성공한 경우
            ...
            saveLog(file_path, f"({response_data[chatbot_helper._meta_data][chatbot_helper._levelNo]}: {response_data[chatbot_helper._meta_data][chatbot_helper._displayName]} - 사용자 입력 채팅 정보: '{userRequest_msg}')")
            ...
            #     saveLog(file_path, f"({master_data[chatbot_helper._levelNo]}: {master_data[chatbot_helper._displayName]} - 사용자 입력 채팅 정보: '{userRequest_msg}')")
            # else:   # 특정 마스터 데이터 값이 존재하지 않는 경우 (예) 아이템 카드 (basicCard, carousel) or 바로가기 그룹 (quickReplies) 
            #     saveLog(file_path, f"(etc: [기술지원 문의 제외 일반 문의] - 사용자 입력 채팅 정보: '{userRequest_msg}')")
...
def dbReset(file_path: str) -> None:
    """
    Description: 임시 로그 초기화

    Parameters: file_path - 아마존 웹서비스 람다 함수 (AWS Lambda Function) -> 임시 로그 텍스트 파일 상대 경로 ('/tmp/botlog.txt')
...
def dbSave(file_path: str, msg: str) -> None:
    """
...
def saveLog(file_path: str, msg: str) -> None:
    """
